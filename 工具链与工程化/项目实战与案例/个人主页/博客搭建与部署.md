...
- 方式 A（主分支 /docs 目录，无需额外分支）：
  - 仓库 Settings → Pages，选择 “Deploy from a branch”
  - Branch 选 `main`，Directory 选 `/docs`
  - 若使用 Vite，将构建目录改为 `docs`：
    ```ts
    // vite.config.ts
    export default {
      base: "/my-site/", // 仓库子路径（如有）
      build: { outDir: "docs" },
    };
    ```
- 方式 B（gh-pages 分支）：
  - 保持默认 `dist` 输出，构建后将 `dist` 发布到 `gh-pages` 分支（可用 GitHub Actions 或 gh-pages CLI）

2. Vercel/Netlify（零配置）

- 连接 Git 仓库，构建命令：`npm run build`
- 发布目录：`dist`（若上一步改为 `docs`，则填写 `docs`）
- 可绑定自定义域名

> 注：以上平台为纯前端静态托管，无需后端/运维介入。若仓库子路径部署（如 /my-site/），在 `vite.config.ts` 设置 `base: '/my-site/'`。
...
onMounted(async () => {
  if (import.meta.env.DEV) {
    // 开发期示例：优先使用本地 mock/静态 JSON；如有后端再通过代理
    try {
...
- 构建后资源 404？GitHub Pages 子路径请设置 `base`；Vercel/Netlify 默认处理 SPA 回退
...
至此，你已经完成从 0 到 1 的个人主页/博客站点：具备主页、文章列表、本地开发、构建与上线的完整闭环。
